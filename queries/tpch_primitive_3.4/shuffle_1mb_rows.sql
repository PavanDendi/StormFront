-- shuffle_1mb_rows.sql
---- QUERY: shuffle_1mb_rows
...
with wide_lineitem
     AS (SELECT *,
                repeat(concat(uuid(),uuid(),uuid(),uuid(),uuid(),uuid(),uuid(),uuid(),uuid(),uuid(),uuid(),uuid(),uuid(),
...
SELECT /*+ SHUFFLE_HASH(t1,t2) */ max(Length(t1.wide_column)) as wc, max(length( t2.wide_column))
FROM   wide_lineitem t1
       JOIN wide_lineitem t2
